...
abstract class Kohana_Jam_Association_Manytomany extends Jam_Association_Collection {
	...
	/**
	 * A custom order for this association, in the form of an array (sort column, order direction)
	 * @var array
	 */
	...
	public function initialize(Jam_Meta $meta, $name)
	{
		...
		if ($this->order_by AND ! is_array($this->order_by))
			throw new Kohana_Exception('order_by must be an array (sort column, order direction)');
	}
	...
	public function collection(Jam_Model $model)
	{
		...
		if ($this->order_by)
		{
			$collection->order_by($this->order_by[0], $this->order_by[1]);
		}
