...
abstract class Kohana_Jam_Meta {
	...
	public function finalize($model)
	{
		...
		$behavior_class = Jam::behavior_prefix().Jam::process_class_name($model);
		...
		if ( ! $this->_collection AND $class = Jam::collection_prefix().Jam::process_class_name($this->_model) AND class_exists($class))
		{
