...
class Kohana_Upload_Server_Rackspace extends Upload_Server
{
	...
	{
		return $this->container()->move_object_to($this->container()->get_object($file), $this->container(), $new_file);
	}

	public function copy($file, $new_file)
	{
		return $this->container()->copy_object_to($file, $this->container(), $new_file);
	}	
	...
	public function webpath($file, $protocol = NULL)
	{
		...
		$protocol = is_bool($protocol) ? (Request::current()->secure() ? 'https' : 'http') : $protocol;

		if ($protocol === 'https' AND isset($this->_config['https_cdn']))
			return rtrim($this->_config['https_cdn'], '/').'/'.$file;
