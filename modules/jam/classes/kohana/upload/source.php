...
abstract class Kohana_Upload_Source {
	...
	public static function process_type_upload(array $data, $directory)
	{
		...
		$filename = Upload_Util::sanitize($data['name']);

		if ( ! move_uploaded_file($data['tmp_name'], Upload_Util::combine($directory, $filename)))
	...
	public static function process_type_stream($stream, $directory, $filename)
	{
		...
		$result_handle = fopen(Upload_Util::combine($directory, Upload_Util::sanitize($filename)), 'w');
